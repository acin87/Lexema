...
import { JWT_PERSISTENT_STATE, LoginForm } from '../../entities/user/types/User.ts';

import { useLoginUserMutation } from '../../entities/user/api/userApi.ts';
import { saveState } from '../../shared/utils/LocalStorage.ts';
                    ...
                    <div className={cn(styles.formWrapper, styles.alignItemsCenter)}>
                        <div className={cn(styles.form, styles.signIn)}>
                            <form id="login" onSubmit={handleSubmitLogin} method="post">
                                ...
                                <Button
                                    variant="contained"
                                    endIcon={isLoading ? <CircularProgress size={25} color="inherit" /> : <SendOutlined />}
                                    sx={{ width: '65%' }}
                                    type="submit"
                                >
                                    {isError ? 'error' : 'Войти'}
